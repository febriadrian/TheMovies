...
struct MovieDetailModel {
    ...
    struct Response {
        ...
        var credits: Credits?
        var recommend: HomeModel.Response?
        var similar: HomeModel.Response?
        ...
        init(data: JSON?) {
            ...
            if let dict = data?["recommendations"].dictionaryObject {
                self.recommend = HomeModel.Response(data: JSON(dict))
            }

            if let dict = data?["similar"].dictionaryObject {
                self.similar = HomeModel.Response(data: JSON(dict))
            }
        }
